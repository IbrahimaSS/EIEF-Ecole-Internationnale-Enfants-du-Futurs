...
type ChildTab = 'notes' | 'pointage' | 'emploi';
...
const ParentEleves: React.FC = () => {
  ...
  const [childTab, setChildTab] = useState<Record<string, ChildTab>>({});
  ...
  const [attendanceCache, setAttendanceCache] = useState<Record<string, QrAttendanceLogEntry[]>>({});
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const tabSelection: Record<string, ChildTab> = {};
        linkedStudents.forEach((student) => {
          semSelection[student.id] = 1;
          tabSelection[student.id] = 'notes';
        });
        setSelectedSemestre(semSelection);
        setChildTab(tabSelection);
  ...
  const loadAttendance = async (studentId: string) => {
    if (attendanceCache[studentId]) return; // déjà chargé
    setAttendanceLoading((prev) => ({ ...prev, [studentId]: true }));
    try {
      const data = await studentCardService.getAttendanceHistory(studentId);
      setAttendanceCache((prev) => ({ ...prev, [studentId]: data }));
    } catch {
      setAttendanceCache((prev) => ({ ...prev, [studentId]: [] }));
    } finally {
                ...
                {isExpanded && (
                  ...
                  >
                    <div className="border-t border-gray-100 dark:border-white/5">
                      {/* Barre d'onglets */}
                      <div className="px-6 pt-4 pb-0 flex flex-wrap items-center justify-between gap-3">
                        <div className="flex bg-gray-100 dark:bg-white/5 rounded-xl p-1 gap-1">
                          {([
                            { key: 'notes' as ChildTab, label: 'Bulletin', icon: <BarChart3 size={12} /> },
                            { key: 'pointage' as ChildTab, label: 'Pointage QR', icon: <QrCode size={12} /> },
                            { key: 'emploi' as ChildTab, label: 'Emploi du temps', icon: <Calendar size={12} /> },
                          ]).map((tab) => (
                            <button
                              key={tab.key}
                              onClick={(e) => {
                                e.stopPropagation();
                                setChildTab((prev) => ({ ...prev, [student.id]: tab.key }));
                                if (tab.key === 'pointage') void loadAttendance(student.id);
                              }}
                              className={cn(
                                'flex items-center gap-1.5 px-3 py-2 rounded-lg text-[11px] font-bold transition-all',
                                (childTab[student.id] ?? 'notes') === tab.key
                        ...
                        {(childTab[student.id] ?? 'notes') === 'notes' && (
                          <div className="flex bg-gray-100 dark:bg-white/5 rounded-xl p-1 gap-1">
                            {semestres.map((sem) => (
                              <button
                                key={sem.key}
                                onClick={(e) => { e.stopPropagation(); setSemestreForChild(student.id, sem.key); }}
                                className={cn(
                                  'px-3 py-2 rounded-lg text-[11px] font-bold transition-all',
                                  currentSem === sem.key
                                    ? 'bg-white dark:bg-gray-800 text-bleu-600 dark:text-or-400 shadow-sm'
                                    : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300',
                                )}
                      ...
                      {/* ── ONGLET POINTAGE QR ── */}
                      {childTab[student.id] === 'pointage' && (
                        <div className="p-6 space-y-4">
                          <div className="flex items-center gap-2 mb-2">
                            <QrCode size={18} className="text-cyan-500" />
                            <h3 className="font-bold text-gray-900 dark:text-white">Historique des pointages QR</h3>
                          </div>
                          {attendanceLoading[student.id] ? (
                            <div className="flex items-center gap-3 py-8 text-gray-400">
                              ...
                              <span className="text-sm font-semibold">Chargement de l'historique…</span>
                            </div>
                          ) : !attendanceCache[student.id] || attendanceCache[student.id].length === 0 ? (
                            <div className="p-6 bg-gray-50 dark:bg-white/5 rounded-2xl text-center text-gray-400">
                              <QrCode size={32} className="mx-auto mb-2 opacity-20" />
                              <p className="text-sm font-semibold">Aucun pointage QR enregistré pour cet élève.</p>
                              <p className="text-xs mt-1">Les arrivées et départs apparaîtront ici après le premier scan.</p>
                            </div>
                          ) : (
                            <>
                              {/* KPIs */}
                              <div className="grid grid-cols-3 gap-3">
                                <div className="rounded-2xl bg-emerald-50 dark:bg-emerald-900/20 px-4 py-3">
                                  <p className="text-2xl font-black text-emerald-700 dark:text-emerald-300 leading-none">{attendanceCache[student.id].length}</p>
                                  <p className="text-[10px] font-bold text-emerald-600/70 mt-0.5 uppercase tracking-wide">Jours pointés</p>
                                </div>
                                <div className="rounded-2xl bg-blue-50 dark:bg-blue-900/20 px-4 py-3">
                                  <p className="text-2xl font-black text-blue-700 dark:text-blue-300 leading-none">
                                    {attendanceCache[student.id].filter(l => l.checkOutTime).length}
                                  </p>
                                  <p className="text-[10px] font-bold text-blue-600/70 mt-0.5 uppercase tracking-wide">Départs scannés</p>
                                </div>
                                <div className="rounded-2xl bg-amber-50 dark:bg-amber-900/20 px-4 py-3">
                                  <p className="text-2xl font-black text-amber-700 dark:text-amber-300 leading-none">
                                    {attendanceCache[student.id].filter(l => !l.checkOutTime).length}
                                  </p>
                                  <p className="text-[10px] font-bold text-amber-600/70 mt-0.5 uppercase tracking-wide">Sans départ</p>
                                </div>
                              </div>
                                ...
                                <table className="w-full text-left text-sm">
                                  <thead className="bg-gray-50 dark:bg-white/5">
                                    <tr>
                                      ...
                                      <th className="px-4 py-3 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Arrivée</th>
                                      <th className="px-4 py-3 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Départ</th>
                                      <th className="px-4 py-3 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Statut</th>
                                    </tr>
                                  </thead>
                                  <tbody className="divide-y divide-gray-50 dark:divide-white/5">
                                    {attendanceCache[student.id].slice(0, 30).map((log) => (
                                      <tr key={log.id} className="hover:bg-gray-50/50 dark:hover:bg-white/[0.02] transition-colors">
                                        ...
                                        <td className="px-4 py-3">
                                          <span className="flex items-center gap-1.5 text-emerald-600 dark:text-emerald-400 font-bold">
                                            <LogIn size={12} /> {fmtTime(log.checkInTime)}
                                          </span>
                                        </td>
                                        <td className="px-4 py-3">
                                          <span className="flex items-center gap-1.5 text-slate-500 font-semibold">
                                            <LogOut size={12} /> {fmtTime(log.checkOutTime)}
                                          </span>
                                        </td>
                                        <td className="px-4 py-3">
                                          {log.status === 'IN_SCHOOL' ? (
                                            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 text-[10px] font-black">
                                              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" /> Présent
                                            </span>
                                          ) : (
                                            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-gray-100 dark:bg-white/10 text-gray-600 dark:text-gray-300 text-[10px] font-black">
                                              <LogOut size={10} /> Sorti
                                            </span>
                                          )}
                                        </td>
                                      </tr>
                        ...
                        <div className="p-6">
                          ...
                          <div className="space-y-3">
                            {scheduleData.length === 0 && (
                              <div className="p-4 bg-gray-50 dark:bg-white/5 rounded-xl">
                                <p className="text-[12px] font-semibold text-gray-500">Aucun cours prévu aujourd'hui.</p>
                              </div>
                            )}
                              ...
                              <div key={cours.id} className="p-3 bg-gray-50 dark:bg-white/5 rounded-xl">
                                <div className="flex items-center justify-between mb-1">
                                  ...
                                  <span className="text-[10px] font-bold text-gray-400 flex items-center gap-1">
                                    <Clock size={10} /> {formatTime(cours.startTime)} - {formatTime(cours.endTime)}
                                  </span>
                                </div>
