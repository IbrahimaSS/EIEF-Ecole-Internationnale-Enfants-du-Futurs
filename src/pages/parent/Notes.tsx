...
const ParentNotes: React.FC = () => {
  ...
  // Charger les notes quand l'enfant ou le semestre change
  useEffect(() => {
    if (!selectedStudentId) return;
    setLoadingNotes(true);
    setError(null);
    studentService.getNotes(selectedStudentId, selectedSem)
      .then(setData)
      .catch(() => setError('Impossible de charger les notes.'))
      .finally(() => setLoadingNotes(false));
  }, [selectedStudentId, selectedSem]);
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-2 border-b border-gray-100 dark:border-white/5">
        ...
        {/* Contrôles : sélecteur enfant + semestre */}
        <div className="flex flex-wrap items-center gap-3 self-start lg:self-center">
          ...
          {students.length > 1 && (
            <div className="relative">
              ...
              >
                <Users size={14} className="text-bleu-500" />
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        s.id === selectedStudentId
                          ? 'bg-bleu-50 dark:bg-bleu-900/30 text-bleu-600'
      ...
      {/* ERREUR */}
      {error && (
        <div className="py-8 text-center text-rouge-600 font-bold text-sm">{error}</div>
      )}

      <AnimatePresence mode="wait">
        <motion.div
          key={`${selectedStudentId}-${selectedSem}`}
          ...
          {loadingNotes ? (
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <Card className="p-6 bg-white dark:bg-gray-900/50 border-none shadow-soft flex items-center gap-6">
                  <div className="p-4 bg-bleu-50 dark:bg-bleu-900/10 rounded-2xl text-bleu-600">
                    <TrendingUp size={24} />
                  </div>
                  <div>
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Moyenne</p>
                    <h3 className="text-2xl font-black text-gray-900 dark:text-white">
                      {formatNote(data.overallAverage)}<span className="text-xs text-gray-400">/20</span>
                    </h3>
                  </div>
                </Card>

                <Card className="p-6 bg-white dark:bg-gray-900/50 border-none shadow-soft flex items-center gap-6">
                  <div className="p-4 bg-or-50 dark:bg-or-900/10 rounded-2xl text-or-600">
                    <Award size={24} />
                  </div>
                  <div>
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Rang</p>
                    <h3 className="text-2xl font-black text-gray-900 dark:text-white">
                      {data.rank}<span className="text-xs text-gray-400 ml-1">/ {data.classSize}</span>
                    </h3>
                  </div>
                </Card>

                <Card className="p-6 bg-white dark:bg-gray-900/50 border-none shadow-soft flex items-center gap-6">
                  <div className="p-4 bg-vert-50 dark:bg-vert-900/10 rounded-2xl text-vert-600">
                    <BarChart3 size={24} />
                  </div>
                  <div>
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Progression</p>
                    <h3 className={cn('text-2xl font-black', data.progression >= 0 ? 'text-vert-600' : 'text-rouge-600')}>
                ...
                <Card className="p-6 bg-white dark:bg-gray-900/50 border-none shadow-soft flex items-center gap-6">
                  <div className="p-4 bg-indigo-50 dark:bg-indigo-900/10 rounded-2xl text-indigo-600">
                    <Download size={24} />
                  </div>
                  <div>
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Absences</p>
                    <h3 className="text-2xl font-black text-gray-900 dark:text-white">
                      {data.absences}<span className="text-xs text-gray-400 ml-1">séances</span>
                    </h3>
                  </div>
                </Card>
              </div>
              ...
              {data.appreciation && (
                <Card className="p-8 border-none shadow-soft bg-gradient-to-r from-bleu-600 to-indigo-700 text-white relative overflow-hidden">
                  ...
                  <div className="relative z-10 max-w-2xl">
                    ...
                    <p className="text-xl font-bold italic leading-relaxed">"{data.appreciation}"</p>
                  </div>
                </Card>
              )}
            </>
            ...
            !error && (
              <div className="py-24 text-center">
                ...
                <p className="text-[13px] text-gray-500 font-semibold max-w-sm mx-auto">
                  Les notes de ce semestre n'ont pas encore été saisies.
                </p>
              </div>
            )
          )}
