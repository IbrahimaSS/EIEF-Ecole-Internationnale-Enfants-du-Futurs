...
import { Card, Badge, StatCard, Button, Avatar, Input } from '../../components/ui';
            ...
            {activeTab === 'systeme' && (
              ...
              >
                <Card className="p-8 border-none shadow-soft bg-white dark:bg-gray-900/50 dark:backdrop-blur-md">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-6">
                      <div className="space-y-2 text-left">
                        <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Nom de l'établissement</label>
                        <Input defaultValue="Écoles Internationales Enfants du Futur" className="w-full h-12 rounded-2xl border-gray-100 dark:border-white/10 dark:bg-white/5 font-bold" />
                      </div>
                      <div className="space-y-2 text-left">
                        <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Slogan / Devise</label>
                        <Input defaultValue="Faisons Plus !" className="w-full h-12 rounded-2xl border-gray-100 dark:border-white/10 dark:bg-white/5 font-semibold italic text-bleu-600 dark:text-or-400" />
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div className="space-y-2 text-left">
                        <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Email Administratif</label>
                        <Input type="email" defaultValue="admin@eief.edu.gn" className="w-full h-12 rounded-2xl border-gray-100 dark:border-white/10 dark:bg-white/5 font-semibold" />
                      </div>
                      <div className="space-y-2 text-left">
                        <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Téléphone</label>
                        <Input type="text" defaultValue="+224 622 00 00 00" className="w-full h-12 rounded-2xl border-gray-100 dark:border-white/10 dark:bg-white/5 font-semibold" />
                      </div>
                  </div>
                </Card>
              </motion.div>
            ...
            {activeTab === 'securite' && (
              ...
              >
                <Card className="p-8 border-none shadow-soft space-y-6 bg-white dark:bg-gray-900/50 dark:backdrop-blur-md">
                  ...
                  <div className="flex w-full items-center justify-between p-6 bg-gray-50 dark:bg-white/5 rounded-[2rem] border border-gray-100 dark:border-white/10 hover:border-red-200 dark:hover:border-rouge-500/30 transition-all cursor-pointer group">
                    ...
                    <Badge variant="error" className="bg-rouge-50 text-rouge-600 dark:bg-rouge-900/20 border-none font-bold uppercase tracking-widest text-[9px] px-4 py-1.5 rounded-full shadow-sm">
                      Désactivé
                    </Badge>
                  </div>
              ...
              >
                <Card className="p-0 border-none shadow-soft overflow-hidden dark:bg-gray-900/50 dark:backdrop-blur-md">
                   <div className="p-6 border-b border-gray-100 dark:border-white/10 flex items-center justify-between text-gray-900 dark:text-white font-bold uppercase tracking-widest text-[11px]">
                      ...
                      <div className="p-2 bg-gray-50 dark:bg-white/5 rounded-xl cursor-pointer hover:bg-gray-100 dark:hover:bg-white/10 transition-colors">
                        ...
                        <div key={log.id} className="p-6 hover:bg-gray-50/80 dark:hover:bg-white/5 transition-colors flex items-center justify-between group cursor-pointer">
                           <div className="flex items-center gap-6">
                              <div className={cn(
                                ...
                                log.statut === 'Alerte' ? "bg-rouge-100 dark:bg-rouge-900/20 text-rouge-600 dark:text-rouge-400" : "bg-gray-50 dark:bg-white/5 text-gray-500 dark:text-gray-400"
                              )}>
                                 {log.statut === 'Alerte' ? <AlertTriangle size={20} /> : <History size={20} />}
                              </div>
                              <div className="text-left font-bold">
                                 <p className="text-gray-900 dark:text-white uppercase tracking-tight text-sm mb-1">{log.action}</p>
                                 <div className="flex items-center gap-3">
                                   <p className="text-[10px] uppercase tracking-widest text-bleu-600 dark:text-bleu-400">{log.utilisateur}</p>
                                   <div className="w-1 h-1 rounded-full bg-gray-300 dark:bg-gray-700" />
                                   <p className="text-[10px] uppercase tracking-widest text-gray-400">{log.date}</p>
                                   <div className="w-1 h-1 rounded-full bg-gray-300 dark:bg-gray-700" />
                                   <Badge className="bg-gray-100 dark:bg-white/5 text-gray-500 border-none px-2 py-0.5 text-[8px]">{log.module}</Badge>
                                 </div>
                              </div>
                           </div>
                           <Badge variant={log.statut === 'Confirmé' ? 'success' : 'error'} className="text-[9px] font-bold uppercase tracking-widest px-3 py-1 shadow-sm border-none">
                             {log.statut}
                           </Badge>
                        </div>
                      ))}
