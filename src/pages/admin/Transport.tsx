      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <h1 className="text-2xl font-black gradient-bleu-or-text uppercase tracking-tighter">Transport Scolaire</h1>
          </div>
          <p className="text-gray-500 dark:text-gray-400 font-medium">Gestion de la flotte, des itinéraires et du ramassage</p>
        </div>
        <div className="flex items-center gap-3">
          <Button variant="outline" className="flex gap-2 text-[10px] font-black uppercase tracking-widest px-5 h-11 dark:border-white/10 dark:text-white">
              ...
              <Card key={i} className="p-0 border-none shadow-soft overflow-hidden group hover:scale-[1.02] transition-all cursor-pointer dark:bg-gray-900/50 dark:backdrop-blur-md">
                <div className={`p-6 border-b border-gray-100 dark:border-white/5 flex items-center justify-between ${
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-black text-gray-900 dark:text-white line-clamp-1">{l.nom}</h3>
                      <p className="text-[10px] font-black uppercase tracking-widest text-gray-500 dark:text-gray-400 italic">{l.bus}</p>
                    </div>
                  </div>
                  <Badge variant={l.statut === 'En route' ? 'success' : l.statut === 'Maintenance' ? 'error' : 'default'} className="text-[9px] font-black px-2 uppercase">
                    {l.statut}
                  </Badge>
                </div>
                <div className="p-6 bg-white dark:bg-transparent space-y-4">
                  <div className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-2 text-gray-500 dark:text-gray-400 font-bold uppercase text-[10px] tracking-wider">
                      <MapPin size={14} className="text-or-500" />
                      Itinéraire principal
                    </div>
                    <span className="font-black text-gray-900 dark:text-white">{l.id}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-[9px] font-black uppercase text-gray-400 tracking-widest">Chauffeur</p>
                      <p className="font-bold text-gray-900 dark:text-white text-sm">{l.chauffeur}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-[9px] font-black uppercase text-gray-400 tracking-widest">Taux d'occupation</p>
                      <p className="font-bold text-gray-900 dark:text-white text-sm">{Math.round((l.inscrits / l.capacite) * 100)}%</p>
                    </div>
                  </div>
                ...
                <div className="p-4 bg-gray-50/50 flex items-center justify-between text-xs font-black uppercase tracking-widest text-gray-400 group-hover:text-bleu-600 group-hover:bg-bleu-50 transition-all">
          ...
          >
            {elevesTransport.map((e, i) => (
              <Card key={i} className="p-4 border-none shadow-soft flex items-center justify-between hover:shadow-lg transition-all">
                ...
                <div className="text-right">
                   <Badge variant="info" className="text-[9px] uppercase font-black">{e.classe}</Badge>
                </div>
              </Card>
            ))}
          </motion.div>
