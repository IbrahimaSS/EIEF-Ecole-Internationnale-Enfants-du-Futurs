      ...
      <div className="relative rounded-[2.5rem] overflow-hidden bg-white dark:bg-gray-900 shadow-soft border border-gray-100 dark:border-white/5">
         ...
         <div className="px-10 pb-10 relative -mt-24 flex flex-col sm:flex-row gap-8 items-center sm:items-end">
            ...
            <div className="flex-1 text-center sm:text-left mb-4">
               <h1 className="text-4xl font-black text-gray-900 dark:text-white tracking-tighter mb-2">{user?.firstName} {user?.lastName}</h1>
               <div className="flex flex-wrap gap-3 justify-center sm:justify-start items-center">
                  <Badge className="bg-bleu-50 text-bleu-600 dark:bg-bleu-900/30 border-none font-black text-[10px] px-3 h-7">Élève ID: #7829</Badge>
                  <Badge className="bg-or-50 text-or-600 dark:bg-or-900/30 border-none font-black text-[10px] px-3 h-7">{user?.classe || '3ème A'}</Badge>
               </div>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
         <div className="space-y-6">
            <Card className="p-8 border-none shadow-soft bg-white dark:bg-gray-900/50">
               <h3 className="text-xs font-black uppercase tracking-widest text-gray-400 mb-6 flex items-center gap-2">Scolarité</h3>
               <div className="space-y-6">
                  <div className="flex items-center gap-4 text-xs font-bold">
                     <div className="p-3 bg-bleu-50 dark:bg-white/5 rounded-2xl text-bleu-600 dark:text-bleu-400"><GraduationCap size={20} /></div>
                     <div>
                        <p className="text-gray-400 text-[10px] uppercase font-black tracking-widest mb-1">Classe actuelle</p>
                        <p className="text-sm text-gray-900 dark:text-white">{user?.classe || '3ème A'}</p>
                     </div>
                  </div>
                  <div className="flex items-center gap-4 text-xs font-bold">
                     <div className="p-3 bg-or-50 dark:bg-white/5 rounded-2xl text-or-600 dark:text-or-400"><Calendar size={20} /></div>
                     <div>
                        <p className="text-gray-400 text-[10px] uppercase font-black tracking-widest mb-1">Date d'inscription</p>
                        <p className="text-sm text-gray-900 dark:text-white">15 Septembre 2024</p>
                     </div>
                  </div>
                  <div className="flex items-center gap-4 text-xs font-bold">
                     <div className="p-3 bg-vert-50 dark:bg-white/5 rounded-2xl text-vert-600 dark:text-vert-400"><BookOpen size={20} /></div>
                     <div>
                        <p className="text-gray-400 text-[10px] uppercase font-black tracking-widest mb-1">N° Matricule</p>
                        <p className="text-sm text-gray-900 dark:text-white">EIEF-2024-991</p>
                     </div>
                  </div>
               </div>
            </Card>
            ...
            <div className="flex gap-6 border-b border-gray-100 dark:border-white/5">
                {(['info', 'securite'] as const).map(t => (
                  ...
                  >
                    ...
                    {activeTab === t && <motion.div layoutId="profile-tab" className="absolute bottom-[-1px] left-0 right-0 h-0.5 bg-current rounded-full" />}
                  </button>
                ))}
            </div>
                     ...
                     <Card className="p-10 border-none shadow-soft bg-white dark:bg-gray-900/50">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                           <div>
                              ...
                              <Input defaultValue={`${user?.firstName} ${user?.lastName}`} className="font-bold text-sm h-12 rounded-xl bg-gray-50/50" />
                           </div>
                           <div>
                              <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 block mb-3 ml-1">Email Élève</label>
                              <Input defaultValue={user?.email} type="email" className="font-bold text-sm h-12 rounded-xl bg-gray-50/50" icon={Mail} />
                           </div>
                           <div>
                              <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 block mb-3 ml-1">Date de naissance</label>
                              <Input defaultValue="12 Mai 2008" className="font-bold text-sm h-12 rounded-xl bg-gray-50/50" icon={Calendar} />
                           </div>
                           <div>
                              <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 block mb-3 ml-1">Téléphone Secours</label>
                              <Input defaultValue={user?.telephone} className="font-bold text-sm h-12 rounded-xl bg-gray-50/50" icon={Phone} />
                           </div>
                           <div className="md:col-span-2">
                              <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 block mb-3 ml-1">Adresse de résidence</label>
                              <Input defaultValue="Kaloum, Avenue de la République" className="font-bold text-sm h-12 rounded-xl bg-gray-50/50" icon={MapPin} />
                           </div>
                        </div>
                     </Card>
                  </motion.div>
                     ...
                     <Card className="p-10 border-none shadow-soft bg-white dark:bg-gray-900/50">
                        ...
                        <div className="space-y-6">
                           <div>
                              ...
                              <Input type="password" placeholder="••••••••" className="font-bold text-sm h-12 rounded-xl" />
                           </div>
                           <div>
