// src/pages/coordinator/Scolarite.tsx
// Orchestrateur du module Scolarité (coordinateur).
...
// Sous-composants
import ScolariteHeader     from './scolarite_module/components/ScolariteHeader';
import StatsCards          from './scolarite_module/components/StatsCards';
import NotificationToast   from './scolarite_module/components/NotificationToast';
import EmploisTab          from './scolarite_module/tabs/EmploisTab';
import NotesTab            from './scolarite_module/tabs/NotesTab';
import PointageTab         from './scolarite_module/tabs/PointageTab';
import StudentCardsTab     from './scolarite_module/tabs/StudentCardsTab';
...
import ScheduleModal       from './scolarite_module/components/modals/ScheduleModal';
import ClassModal          from './scolarite_module/components/modals/ClassModal';
import SubjectModal        from './scolarite_module/components/modals/SubjectModal';
import AcademicYearModal   from './scolarite_module/components/modals/AcademicYearModal';
import ClassDetailModal    from './scolarite_module/components/modals/ClassDetailModal';
import GradesModal         from './scolarite_module/components/modals/GradesModal';
import DeleteConfirmModal  from './scolarite_module/components/modals/DeleteConfirmModal';
import TeacherHistoryModal from './scolarite_module/components/modals/TeacherHistoryModal';
import StudentHistoryModal from './scolarite_module/components/modals/StudentHistoryModal';

// Hooks
import { useNotif }            from './scolarite_module/hooks/useNotif';
import { useScolariteData }    from './scolarite_module/hooks/useScolariteData';
import { useGradeEntry }       from './scolarite_module/hooks/useGradeEntry';
import { useStudentGrades }    from './scolarite_module/hooks/useStudentGrades';
import { useClassSchedules }   from './scolarite_module/hooks/useClassSchedules';
import { useAttendance }       from './scolarite_module/hooks/useAttendance';
import { useTeacherHistory }   from './scolarite_module/hooks/useTeacherHistory';
import { useStudentAttendanceHistory } from './scolarite_module/hooks/useStudentAttendanceHistory';
import { useStudentBulletin } from './scolarite_module/hooks/useStudentBulletin';

// API + utils
import { apiFetch }              from './scolarite_module/api';
import { printSchedule }         from './scolarite_module/utils/printSchedule';
import { printTeacherReport }    from './scolarite_module/utils/printTeacherReport';
import { printBulletinPeriode, printBulletinAnnuel } from './scolarite_module/utils/printBulletin';

// Types & constantes
import { empty } from './scolarite_module/constants';
import {
  AcademicYearForm,
  ...
  TabId,
  TeacherResponse,
} from './scolarite_module/types';

// Le coordinateur a toujours ces droits
const ALLOW_CATALOG_MANAGEMENT = false;
...
const CoordinatorScolarite: React.FC = () => {
  ...
  const handlePrintBulletinPeriode = async (period: number) => {
    ...
    else onError('Impossible de générer le bulletin.');
  };
  ...
  const searchResultCount =
    activeTab === 'emplois' ? filteredClasses.length : filteredStudents.length;
  ...
  const handleSubmitClass = async () => {
    ...
    try {
      ...
      onError(
        e?.message ?? (editingClassId ? 'Erreur modification classe.' : 'Erreur création classe.'),
      );
    } finally {
      setSubmitting(false);
    }
        ...
        <div className="flex items-start gap-3 rounded-[1.75rem] border border-amber-200 bg-amber-50/95 p-4 text-sm text-amber-700 shadow-[0_18px_40px_-28px_rgba(245,158,11,0.65)] dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-300">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs leading-relaxed">
              ...
              {failedEndpoints.map(f => f.split(' (')[0]).join(', ')}.
