...
const TeacherCommunication: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'annonces' | 'messagerie' | 'forum'>('messagerie');
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null);
    try {
      const [a, t] = await Promise.all([getAnnouncements('TEACHER'), getForumTopics()]);
      setAnnonces(a);
      setTopics(t);
    } catch (err: any) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs font-bold uppercase tracking-widest text-vert-700 dark:text-or-400">
              Échanges avec parents · élèves · direction
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
      ...
      <AnimatePresence mode="wait">
        {activeTab === 'annonces' && (
          ...
          >
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-black text-gray-900 dark:text-white">Annonces de la direction</h2>
              <button
                onClick={loadData}
            ...
            {loading && annonces.length === 0 ? (
              ...
              <Card className="p-12 text-center border-none shadow-soft dark:bg-gray-900/50">
                ...
                <p className="text-sm font-bold text-gray-500">Aucune annonce.</p>
              </Card>
            ) : (
                ...
                {annonces.map((ann) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-sm font-medium text-gray-600 dark:text-gray-300 leading-relaxed">{ann.content}</p>
        ...
        {activeTab === 'messagerie' && (
          <motion.div key="messagerie" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <MessagingPanel
              title="Messagerie"
              subtitle="Échangez avec vos élèves, leurs parents et la direction"
              height={650}
            />
          </motion.div>
        )}
          ...
          >
            ...
            {topics.length === 0 ? (
              <Card className="p-12 text-center border-none shadow-soft dark:bg-gray-900/50">
                ...
                <p className="text-sm font-bold text-gray-500">Aucun sujet pour le moment.</p>
              </Card>
            ) : (
